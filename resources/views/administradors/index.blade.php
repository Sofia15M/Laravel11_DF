<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8">
                <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="mb-4">
                        <form action="{{ route('administradors.index') }}" method="GET">
                    ...
                    <div class="mb-4">
                        @if (auth()->user()->id_rol == 1)
                            <a href="{{ route('administradors.create') }}" class="bg-Azul3 dark:bg-Azul3 hover:bg-Azul2 dark:hover:bg-Azul4 text-white font-bold py-2 px-4 rounded mr-2 mb-2 float-left" title="Añadir registro">
                        ...
                        @endif
                        <a href="{{ route('administradors.inactive') }}" class="bg-naranja dark:bg-naranja1 hover:bg-naranja1 dark:hover:bg-naranja text-white font-bold py-2 px-4 rounded mr-2 mb-2 float-left" title="Desativados">
                        ...
                        <a href="{{ route('administradors.pdf')}}" class="bg-Azul3 dark:bg-Azul3 hover:bg-Azul2 dark:hover:bg-Azul4 text-white font-bold py-2 px-4 rounded float-right" title="Imprimir" target="_blank">
                                ...
                                @if (auth()->user()->id_rol == 1)
                                    <td class="border px-4 py-2 text-center">
                                        <div class="flex justify-center">
                                                <a href="{{ route('administradors.edit', $administrador->ID_Administrador) }}" class="bg-verde dark:bg-verde1 hover:bg-verde dark:hover:bg-verde1 text-white font-bold py-2 px-4 rounded mr-2" title="Editar">
                                                    <svg class="h-5 w-5 text-white" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                                                    </svg>
                                                </a>
                                                <button type="button" onclick="confirmDeactivate('{{ $administrador->ID_Administrador }}')" class="bg-naranja dark:bg-naranja1 hover:bg-naranja1 dark:hover:bg-naranja text-white font-bold py-2 px-4 rounded mr-2" title="Desactivar">
                    ...
                    <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                        <div class="flex flex-1 justify-between sm:hidden">
                            <a href="{{ $administradors->previousPageUrl() }}" class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Previous</a>
                            <a href="{{ $administradors->nextPageUrl() }}" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Next</a>
                        </div>
                        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Mostrando
                                    <span class="font-medium">{{ $administradors->firstItem() }}</span>
                                    a
                                    <span class="font-medium">{{ $administradors->lastItem() }}</span>
                                    de
                                    <span class="font-medium">{{ $administradors->total() }}</span>
                                    resultados
                                </p>
                            </div>
                            <div>
                                <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                                    <a href="{{ $administradors->previousPageUrl() }}" class="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
                                        <span class="sr-only">anterior</span>
                                        <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                            <path fill-rule="evenodd" d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02z" clip-rule="evenodd" />
                                        </svg>
                                    </a>
                                    @for ($i = 1; $i <= $administradors->lastPage(); $i++)
                                        <a href="{{ $administradors->url($i) }}" class="relative inline-flex items-center px-4 py-2 text-sm font-semibold {{ $administradors->currentPage() == $i ? 'bg-Azul3 text-white' : 'text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50' }}">{{ $i }}</a>
                                    @endfor
                                    <a href="{{ $administradors->nextPageUrl() }}" class="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
    ...
    <script>
        function confirmDeactivate(id) {
            Swal.fire({
                title: "¿Estás seguro?",
                text: "¡Esto desativara el administrador!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('administrador.index', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        Swal.fire({
                            title: "¡Desativado!",
                            text: "El administrador ha sido desativado.",
                            icon: "success"
                        }).then(() => {
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            title: "Error",
                            text: `Hubo un problema al desativar el administrador: ${error.message}`,
                            icon: "error"
                        });
                    });
                }
