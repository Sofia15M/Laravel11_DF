<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8">
                <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="mb-4">
                        <form action="{{ route('vigilantes.index') }}" method="GET">
                    ...
                    <div class="mb-4">
                        @if (auth()->user()->id_rol == 1)
                            <a href="{{ route('vigilantes.create') }}" class="bg-Azul3 dark:bg-Azul3 hover:bg-Azul2 dark:hover:bg-azul4 text-white font-bold py-2 px-4 rounded mr-2 mb-2 float-left" title="Añadir registro">
                        ...
                        @endif
                        <a href="{{ route('vigilantes.inactive') }}" class="bg-naranja dark:bg-naranja1 hover:bg-naranja1 dark:hover:bg-naranja text-white font-bold py-2 px-4 rounded mr-2 mb-2 float-left" title="Desativados">
                        ...
                        <a href="{{ route('vigilantes.pdf')}}" class="bg-Azul3 dark:bg-Azul3 hover:bg-2 dark:hover:bg-azul4 text-white font-bold py-2 px-4 rounded float-right" title="Imprimir" target="_blanl">
                                ...
                                @if (auth()->user()->id_rol == 1)
                                    <td class="border px-4 py-2 text-center">
                                        <div class="flex justify-center">
                                            <a href="{{ route('vigilantes.edit', $vigilante->ID_Vigilante) }}" class="bg-verde dark:bg-verde1 hover:bg-verde dark:hover:bg-verde1 text-white font-bold py-2 px-4 rounded mr-2" title="Editar">
                                                <svg class="h-5 w-5 text-white" <svg  width="24"  height="24"  viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                            </a>
                                            <button type="button" onclick="confirmDeactivate('{{ $vigilante->ID_Vigilante }}')" class="bg-naranja dark:bg-naranja1 hover:bg-naranja1 dark:hover:bg-naranja text-white font-bold py-2 px-4 rounded mr-2" title="Desactivar">
    ...
    <script>
        function confirmDeactivate(id) {
            Swal.fire({
                title: "¿Estás seguro?",
                text: "¡Esto desativara el vigilante!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('vigilante.index', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        Swal.fire({
                            title: "¡Desativado!",
                            text: "El vigilante ha sido desativado.",
                            icon: "success"
                        }).then(() => {
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            title: "Error",
                            text: `Hubo un problema al desativar el vigilante: ${error.message}`,
                            icon: "error"
                        });
                    });
                }
