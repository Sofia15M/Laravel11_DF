<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight text-center">
            {{ __('Lista De Empleados Desactivados') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8">
                <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="mb-4">
                        <form action="{{ route('empleados.inactive') }}" method="GET">
                                ...
                                @if (auth()->user()->id_rol == 1)
                                    <td class="border px-4 py-2 text-center">
                                        <div class="flex justify-center">
                                            <button type="button" onclick="confirmActivo('{{ $empleado->ID_PersonalL }}')" class="bg-verde dark:bg-verde1 hover:bg-verde1 dark:hover:bg-verde text-white font-bold py-2 px-4 rounded mr-2" title="Activar">
    ...
    <script>
        function confirmActivo(id) {
            Swal.fire({
                title: "¿Estás seguro?",
                text: "¡Esto activara a el empleado!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('empleado.inactive', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        headers: {
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            title: "Error",
                            text: `Hubo un problema al activar el empleado: ${error.message}`,
                            icon: "error"
                        });
                    });
                }
