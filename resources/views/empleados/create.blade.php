<x-app-layout>
                    ...
                    {{ __('Crear Nuevo Empleado') }}
                        ...
                        <form method="POST" action="{{ route('empleados.store') }}" class="max-w-sm mx-auto" enctype="multipart/form-data">
                            ...
                            <div class="mb-5">
                                ...
                                <span id="error-id-empleado" class="text-red-600 mt-2 text-sm"></span>
                            </div>
                            ...
                            <div class="mb-5">
                                ...
                                <select name="Cargo_PersonalL" id="Cargo_PersonalL" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                    ...
                                    <option value="Jefe de Jardinería o Paisajista">Jefe de Jardinería o Paisajista</option>
                                    <option value="Coordinador de Eventos o Relaciones Comunitarias">Coordinador de Eventos o Relaciones Comunitarias</option>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var empleadosIds = @json($empleadosIds);

            // Imprimir los IDs en la consola para verificar
            console.log("IDs de empleados existentes:", empleadosIds);

            // Validar si el ID ya existe
            document.getElementById('ID_PersonalL').addEventListener('input', function() {
                var idEmpleado = this.value.trim();

                // Convertir a string para garantizar que la comparación sea correcta
                if (empleadosIds.map(String).includes(idEmpleado)) {
                    document.getElementById('error-id-empleado').textContent = 'Este vigilante ya existe.';
                } else {
                    document.getElementById('error-id-empleado').textContent = '';
                }
            });
        });
